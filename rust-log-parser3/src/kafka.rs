...
//! # Отправка данных
//!
//! Функция [`send_log_entries_to_kafka`] сериализует срез [`LogEntry`] в JSON-массив
...
/// через `let (a, b, ..)`, так как тот же таймаут используется внутри [`send_log_entries_to_kafka`]
...
pub async fn send_log_entries_to_kafka(
    brokers: &str,
    topic: &str,
    v: &[LogEntry],
