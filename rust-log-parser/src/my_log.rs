//! Логи работы программы: неблокирующая запись в `parser_logs/` (крейт `tracing-appender`).
...
const LOG_DIR: &str = "parser_logs";
const LOG_FILE_PREFIX: &str = "rust-log-parser";

/// Подключает `tracing` к ротируемому файлу в `parser_logs/` через очередь в отдельном потоке.
///
