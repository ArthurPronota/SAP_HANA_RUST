//! Логи работы программы: неблокирующая запись в `hana_logs/` (крейт `tracing-appender`).
...
const LOG_DIR: &str = "hana_logs";
const LOG_FILE_PREFIX: &str = "rust-log-hana";

/// Подключает `tracing` к ротируемому файлу в `hana_logs/` через очередь в отдельном потоке.
///
