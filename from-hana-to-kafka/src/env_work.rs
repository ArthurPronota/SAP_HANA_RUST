...
/// Однократная загрузка `.env` (для модулей, где нужны переменные после `dotenv`).
...
/// Имя переменной окружения для ODBC-строки подключения к SAP HANA.
const HANA_ODBC_CONNECTION_ENV: &str = "HANA_ODBC_CONNECTION";

/// Значение `HANA_ODBC_CONNECTION` из `.env` или окружения процесса.
pub static HANA_ODBC_CONNECTION: LazyLock<String> = LazyLock::new(|| {
    load_dotenv();
    let key = HANA_ODBC_CONNECTION_ENV;
    match std::env::var(key) {
        Ok(value) => value,
        Err(e) => {
            tracing::error!(
